...
# --------------------------------------------------------------------------------
#----------Create trait value vectors for partial effect plots---------------------
#---------------------------------------------------------------------------------
...
for(Model_type in Model_types){
  ...
  #----------------------------------------------------------------
  #------Run models using all pseudoabsences and presences --------
  #----------------------------------------------------------------
  #Prepare rasters
  #rasters<-terra::rast(outfile)
  predictors<-names(rasters)
  
  #Use all pseudoabsences and presences to train the final model
  train.dataset <- data.frame( PA = c( rep(1,nrow(occurrence.records)) , rep(0,nrow(background)) ) , terra::extract( rasters, rbind( occurrence.records, background), ID=F ) )
  ...
  terra::plot(final.prediction.brt)
  ...
  #Reclassify
  final.prediction.brt.reclassified<- reclassify.predicted(final.prediction.brt,occurrence.records,absences,method="direct.reclass",reclass.threshold=mtp_threshold)
  plot(final.prediction.brt.reclassified)
  
  
  #--------------------------------------------------------------------
  #--------------------------Save predictions -------------------------
  #--------------------------------------------------------------------
  results_folder<-file.path("results", "final_brt_predictions", "final_suitability_maps","Present")
  if(!dir.exists(results_folder))dir.create(results_folder,recursive=TRUE)
  terra::writeRaster(final.prediction.brt, file.path(results_folder,paste0("Final_prediction_brt_",Model_type,"_present.tif")), overwrite=T)
  terra::writeRaster(final.prediction.brt.reclassified, file.path(results_folder, paste0("Final_prediction_brt_reclassified_",Model_type,"_present.tif")), overwrite=T)
  ...
  varimp_folder<-file.path("results", "final_brt_predictions", "variable_importance")
  ...
  #Export
  write.csv2(varimp_df, file.path(varimp_folder, paste0(Model_type, "_variable_importance.csv")), row.names=FALSE)
  ...
  # ------------------------------------------------------------------------------------
  #Create folder for results
  response_folder<-file.path("results", "final_brt_predictions", "response_curves")
  ...
  for( var.n in 1:length(names(rasters))) {
    response_curve<-model.plot(model_type=Model_type,
                               model=model,
                               training.data = train.dataset[-1],
  ...
  #Export
  write.csv2(response_df, file.path(response_folder, paste0(Model_type, "_response_curves.csv")), row.names=FALSE)
  ...
  #Create folder
  present_mess_folder<-file.path("results", "final_brt_predictions", "MESS_maps","Present")
  if(!dir.exists(present_mess_folder))dir.create(present_mess_folder,recursive=TRUE)
  
  #Write rasters
  terra::writeRaster(mess_present,file.path(present_mess_folder,paste0("Final_mess_map_",Model_type,"_present.tif")),overwrite=T)
  ...
  # ----------------------------------------------------------------------------
  #Create folder for results
  deptharea_folder<-file.path("results", "final_brt_predictions", "depth_area_overview", "Present")
  ...
  for(Region in c("Full_range", "MedSea")){
    ...
    #--------------------
    #Export
    write.csv(area_depth, file.path(deptharea_folder, paste0("depth_area_info_",Region,"_",Model_type,"_present.csv")), row.names = F)
  }
  ...
  for (predict.time in predict.times){
    ...
    cat("Predicting for", predict.time)
    ...
    fut_log_folder<-file.path("results", "final_brt_predictions", "final_suitability_maps", predict.time, "Logs")
    if(!dir.exists(fut_log_folder))dir.create(fut_log_folder,recursive=TRUE)
    
    Overview<-data.frame(Rasters=names(future.rasters),Raster_file_names=rasters.names.future,Resp=variable.monotonic.response,  Time=predict.time)
    write.csv2(Overview, file.path(fut_log_folder, paste0(Model_type,"_raster_input_log_",predict.time,".csv")), row.names = FALSE)
    Overview
    ...
    plot(Predicted_future_distribution)
    Reclassified_future_distribution<- reclassify.predicted(Predicted_future_distribution,occurrence.records,absences,method="direct.reclass",reclass.threshold=mtp_threshold)
    plot(Reclassified_future_distribution)
    
    
    #--------------------------------------------------------------------
    #----------------------Save future prediction layers ---------------
    #--------------------------------------------------------------------
    fut_pred_folder<-file.path("results", "final_brt_predictions", "final_suitability_maps", predict.time)
    if(!dir.exists(fut_pred_folder))dir.create(fut_pred_folder,recursive=TRUE)
    
    terra::writeRaster(Predicted_future_distribution,file.path(fut_pred_folder,paste0("Final_prediction_brt_",Model_type,"_",predict.time,".tif")),overwrite=T)
    terra::writeRaster(Reclassified_future_distribution,file.path(fut_pred_folder,paste0("Final_prediction_brt_reclassified_",Model_type,"_",predict.time,".tif")),overwrite=T)
    ...
    #Create folder
    fut_mess_folder<-file.path("results", "final_brt_predictions", "MESS_maps", predict.time)
    if(!dir.exists(fut_mess_folder))dir.create(fut_mess_folder,recursive=TRUE)
    
    #Write rasters
    terra::writeRaster(mess_future,file.path(fut_mess_folder,paste0("Final_mess_map_",Model_type,"_",predict.time,".tif")),overwrite=T)
    ...
    # ----------------------------------------------------------------------------
    #Create folder for results
    future_deptharea_folder<-file.path("results", "final_brt_predictions", "depth_area_overview", predict.time)
    ...
    for(Region in c("Full_range", "MedSea")){
      ...
      #--------------------
      #Export
      write.csv(area_depth_future, file.path(future_deptharea_folder, paste0("depth_area_info_",Region,"_",Model_type,"_",predict.time,".csv")), row.names = F)
    }
